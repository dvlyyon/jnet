...
public class FileUtils {
	...
	public static BufferedReader openBufferedReader (String fileName, RunState state) {
		File file = new File(fileName);
		BufferedReader reader=null;
		if (!file.exists() || !file.canRead()) {
			state.setResult(State.ERROR);
			state.setErrorInfo("file "+ fileName + "don't exist or cannot be read");
			return null;
		}
		if (file.isDirectory()) {
			state.setResult(State.ERROR);
			state.setErrorInfo("File "+fileName+" is a directory");
			return null;
		}
		try {
			...
			state.setErrorInfo("Excep is thrown when creating reader with fileName "+fileName);
			state.setExp(e);
			return null;
		}
		return reader;
	...
	public static boolean openFile(String fileName, RunState state) {
		File file = new File(fileName);
		if (!file.exists() || !file.canRead()) {
			state.setResult(State.ERROR);
			state.setErrorInfo("file "+ fileName + "don't exist or cannot be read");
    ...
    public static String getFileAsString(String fileName)
        ...
        protected static final Log s_logger = LogFactory.getLog(FileUtils.class.getName());
